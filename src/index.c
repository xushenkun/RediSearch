...
inline int IR_GenericRead(IndexReader *ir, t_docId *docId, float *freq, u_char *flags,
  ...
  *docId = ReadVarint(ir->buf) + ir->lastId;
  ...
  size_t offsetsLen = len - (ir->buf->offset - pos);
...
int IR_SkipTo(void *ctx, u_int32_t docId, IndexResult *hit) {
  ...
  if (hit->docId == ir->lastId && docId == hit->docId && ir->lastId && IR_HasNext(ir)) {

    printf("ir %s, skipto %d, last id %d, AUTO OK\n", ir->term->str, docId, ir->lastId);
    return INDEXREAD_OK;
  }
  hit->numRecords = 0;
  SkipEntry *ent = SkipIndex_Find(ir->skipIdx, docId, &ir->skipIdxPos);
  printf("ir %s, skipto %d, last id %d, ent %d\n", ir->term->str, docId, ir->lastId,
         ent ? ent->docId : 0);
  ...
  if (ent != NULL || ir->skipIdx == NULL || ir->skipIdx->len == 0 ||
    ...
    do {
      ...
      if (rc == INDEXREAD_OK || readId > docId) {
        ...
        IR_Read(ir, hit);
        printf("ir %s seekto %d, read %d, rc %d\n", ir->term->str, docId, readId, rc);
        return rc;
      }
      lastId = readId;
...
size_t IW_WriteEntry(IndexWriter *w, ForwardIndexEntry *ent) {
  ...
  // printf("docId %d, score %f, quantizedScore %d, delta %d, offsetsSz %zd, len %zd\n", docId,
  // freq,
  //        quantizedScore, docId - w->lastId, offsetsSz, len);

  // Write the offsets vector
...
int II_Read(void *ctx, IndexResult *hit) {
  ...
  do {
    ...
    for (i = 0; i < ic->num; i++) {
      ...
      printf("II %d, read docId %d, last docId was %d, rc %d\n", i, h->docId, ic->lastDocId, rc);
