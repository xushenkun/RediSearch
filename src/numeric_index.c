...
int NR_Read(void *ctx, RSIndexResult **r) {
  ...
  printf("numeric iterator %s read docId %d (%f) ok!\n", it->nf->fieldName, it->lastDocId,
         lastValue);
...
int NR_SkipTo(void *ctx, u_int32_t docId, RSIndexResult **r) {
  ...
  NumericRangeIterator *it = ctx;
  printf("nr %s skipto %d\n", it->nf->fieldName, docId);
  ...
  if (it->lastDocId != docId) {
    printf("NR %s skipto %d returning notfound, last docId %d\n", it->nf->fieldName, docId,
           it->lastDocId);
    *r = it->rec;
